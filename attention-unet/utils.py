...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        transform_target = transforms.Compose(
                            [#transforms.ToTensor(), 
                                transforms.Resize((352, 352)),  transforms.Grayscale(), transforms.ToTensor()]
                                )
        import pudb; pu.db 
        ...
        image = transforms.Resize((352, 352), antialias=True)
        image = cv2.resize(image, self.mask_resize,fx=0, fy=0, interpolation = cv2.INTER_NEAREST)
        mask = cv2.resize(mask, self.mask_resize,fx=0, fy=0, interpolation = cv2.INTER_NEAREST)
        import pudb; pu.db
        if len(image.shape) == 3:
...
class ToTensor(object):
    ...
    def __call__(self, sample, mask_resize=None, image_resize=None):
        ...
        cv2.imwrite("mm.png", mask[0])
        #return {'image': torch.from_numpy(image),
        #        'mask': torch.from_numpy(miask)}
        return {'image': image,
                'mask': mask}
...
def get_data_loaders(data_dir, image_folder='test_with_inu', mask_folder='test/MS', batch_size=4):
    from torchvision import transforms
    data_transforms = {
#        Resize((592, 576), (592, 576)),
    ...
    from torchvision import transforms
    '''
    transform_input4test = {'training':transforms.Compose(
            [transforms.ToTensor(),
